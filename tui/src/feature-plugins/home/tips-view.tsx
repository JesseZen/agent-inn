...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("codex-proxy.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}codex-proxy.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/codex-proxy/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.codex-proxy/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.codex-proxy/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.codex-proxy/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.codex-proxy/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenCode from reading sensitive files",
  "Use {highlight}codex-proxy run{/highlight} for non-interactive scripting",
  "Use {highlight}codex-proxy --continue{/highlight} to resume the last session",
  "Use {highlight}codex-proxy run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}codex-proxy serve{/highlight} for headless API access to OpenCode",
  "Use {highlight}codex-proxy run --attach{/highlight} to connect to a running server",
  "Run {highlight}codex-proxy upgrade{/highlight} to update to the latest version",
  "Run {highlight}codex-proxy auth list{/highlight} to see all configured providers",
  "Run {highlight}codex-proxy agent create{/highlight} for guided agent creation",
  "Use {highlight}/codex-proxy{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}codex-proxy github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/codex-proxy fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/cap{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.codex-proxy/themes/{/highlight} directory",
  ...
  "Run {highlight}codex-proxy debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/codex-proxy{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with CodexProxy Zen for curated, tested models",
