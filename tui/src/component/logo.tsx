...
function logoGlyphColors(ctx: LogoContext, style: LogoStyle | undefined) {
...
export function Logo(props: { shape?: LogoShape; style?: LogoStyle; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const renderLine = (
    ...
    return Array.from(line).map((char, i) => {
      ...
      const glyphId = current.MAP.glyph.get(key(off + i, y))
      const charInk = glyphId !== undefined ? glyphColors?.get(glyphId) ?? ink : ink
      const shadow = tint(theme.background, charInk, 0.25)

      const h = field(off + i, y, frame, current)
      const charLit = lit(char)
      // Sub-pixel sampling: cells are 2 pixels tall. Sample at top (y*2) and bottom (y*2+1) pixel rows.
      const pulseTop = state ? idle(off + i, y * 2, frame, current, state) : { glow: 0, peak: 0, primary: 0 }
      const pulseBot = state ? idle(off + i, y * 2 + 1, frame, current, state) : { glow: 0, peak: 0, primary: 0 }
      ...
      const n = wave(off + i, y, frame, charLit, current) + h + (charLit ? sweepGlow(off + i, y, frame, current) : 0)
      const s = wave(off + i, y, dusk, false, current) + h
      const p = charLit ? pick(off + i, y, frame, current) : 0
      const e = charLit ? trace(off + i, y, frame, current) : 0
      const b = charLit ? bloom(off + i, y, frame, current) : 0
      const q = shimmer(off + i, y, frame, current)
          ...
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), ink, true, 0, frame(), dusk(), idleState(), glyphColors())}
              </box>
              <box flexDirection="row">
                {renderLine(
