...
export interface DialogSelectProps<T> {
  ...
  actions?: {
    ...
    title: string
    side?: "left" | "right"
    hidden?: boolean | ((option: DialogSelectOption<T> | undefined) => boolean)
...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  const shownActions = createMemo(() => actions().filter((item) => !isActionHidden(item)))
      ...
      bindings: [
        ...
        ...(visible.length
          ? [
              {
                key: "tab",
                desc: t("dialog.nextAction"),
                group: t("category.dialog"),
                cmd: () => moveAction(1),
              },
              {
                key: "shift+tab",
                desc: t("dialog.previousAction"),
                group: t("category.dialog"),
                cmd: () => moveAction(-1),
              },
            ]
  ...
  function isActionHidden(item: Action) {
    return typeof item.hidden === "function" ? item.hidden(selected()) : item.hidden
  }
          ...
          fallback={
            props.emptyView ?? (
              <box paddingLeft={4} paddingRight={4} paddingTop={1}>
                <text fg={theme.textMuted}>{t("dialog.noResults")}</text>
              </box>
            )
          }
        >
