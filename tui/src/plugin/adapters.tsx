import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@codex-proxy/plugin/tui"
...
import type { useCodexProxyKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCodexProxyKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCodexProxyModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCodexProxyModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
